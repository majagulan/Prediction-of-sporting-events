    ...
    for x = 1:n %po igrama
        ...
        r_novi(2,1) = (r_stari(2,1) + (K*g(1,y)*(S(1,2) - mi(1,2))))+100; %80 poena se dodaje domacinu 
    ...
    verov_nereseno=1/sqrt(2*pi*exp(1))*exp((-1)*((dr/200).^2)/(2*(exp(1)).^2));
    ...
    verov_nereseno2=1/sqrt(2*pi*exp(1))*exp((-1)*((dr/200).^2)/(2*(exp(1)).^2));
